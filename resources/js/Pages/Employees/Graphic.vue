...
props.data.map( (row) =>(
    deptoyempl.value.push(row.name),
    deptoyempl.value.push(Math.random()),
    colors.value.push("rgb("+random()+","+random()+","+random()+")")
))
chartOptions.value= { responsive:true}
chartData.value = {
    labels:deptoyempl.value,
    datasets:[
        {label:'Empleados', data:deptoyempl.value, backgroundColor:colors}         
    ]
}
</script>
